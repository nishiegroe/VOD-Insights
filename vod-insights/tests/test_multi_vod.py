...
class TestMultiVodAPI:
    ...
    def test_playback_play_api(self):
        """Test PUT /api/sessions/multi-vod/{sessionId}/playback with play action."""
        vod_file = self._create_fake_vod_file()
        ...
        with patch('app.multi_vod_api._get_vod_metadata') as mock_metadata:
            ...
            # Play
            play_resp = self.client.put(
                f'/api/sessions/multi-vod/{session_id}/playback',
                json={"action": "play"},
                content_type='application/json'
            )
            
            assert play_resp.status_code == 200
            data = play_resp.get_json()
            assert data['ok']
            assert data['session']['global_playback_state'] == 'playing'
    
    def test_playback_pause_api(self):
        """Test PUT /api/sessions/multi-vod/{sessionId}/playback with pause action."""
        vod_file = self._create_fake_vod_file()
        ...
        with patch('app.multi_vod_api._get_vod_metadata') as mock_metadata:
            ...
            data = pause_resp.get_json()
            assert data['ok']
            assert data['session']['global_playback_state'] == 'paused'
    
    def test_playback_seek_api(self):
        """Test PUT /api/sessions/multi-vod/{sessionId}/playback with seek action."""
        vod_file = self._create_fake_vod_file()
        ...
        with patch('app.multi_vod_api._get_vod_metadata') as mock_metadata:
            ...
            # Seek to 150.5 seconds
            seek_resp = self.client.put(
                f'/api/sessions/multi-vod/{session_id}/playback',
                json={"action": "seek", "timestamp": 150.5},
                content_type='application/json'
            )
            ...
            # Verify all VODs' current_time is updated
            for vod in data['session']['vods']:
                assert vod['current_time'] == 150.5
    
    def test_playback_seek_with_offsets_api(self):
        """Test seek respects VOD offsets."""
        vod_file = self._create_fake_vod_file()
        ...
        with patch('app.multi_vod_api._get_vod_metadata') as mock_metadata:
            ...
            # Set offsets: v1 at 0, v2 at -50
            self.client.put(
                f'/api/sessions/multi-vod/{session_id}/offsets',
                json={
                    "offsets": {"v1": 0.0, "v2": -50.0},
                    "source": "manual"
                },
                content_type='application/json'
            )
            ...
            seek_resp = self.client.put(
                f'/api/sessions/multi-vod/{session_id}/playback',
                json={"action": "seek", "timestamp": 200.0},
                content_type='application/json'
            )
            ...
            # Global time should be 200
            assert data['session']['global_time'] == 200.0
            
            # v1 (offset 0) should be at 200
            v1 = next(v for v in data['session']['vods'] if v['vod_id'] == 'v1')
            assert v1['current_time'] == 200.0
            
            # v2 (offset -50) should be at 200 - (-50) = 250
            v2 = next(v for v in data['session']['vods'] if v['vod_id'] == 'v2')
            assert v2['current_time'] == 250.0
    
    def test_playback_invalid_action_api(self):
        """Test playback with invalid action returns 400."""
        vod_file = self._create_fake_vod_file()
    ...
    def test_playback_seek_without_timestamp_api(self):
        """Test seek without timestamp returns 400."""
        vod_file = self._create_fake_vod_file()
        ...
        with patch('app.multi_vod_api._get_vod_metadata') as mock_metadata:
            ...
            data = resp.get_json()
            assert not data['ok']
            assert "timestamp required" in data['error']
    
    def test_playback_session_not_found_api(self):
        """Test playback with non-existent session returns 404."""
        resp = self.client.put(
            '/api/sessions/multi-vod/nonexistent/playback',
            json={"action": "play"},
            content_type='application/json'
        )
    ...
    def test_playback_persists_session(self):
        """Test that playback changes are persisted to disk."""
        vod_file = self._create_fake_vod_file()
        ...
        with patch('app.multi_vod_api._get_vod_metadata') as mock_metadata:
            ...
            # Load from disk
            loaded_session = MultiVodSessionManager.load_session(session_id)
            assert loaded_session is not None
            assert loaded_session.global_playback_state == 'playing'
...
def run_tests():
    ...
    test_api.setup_method()
    test_api.test_playback_play_api()
    test_api.teardown_method()
    print("✓ Playback play API")
    
    test_api.setup_method()
    test_api.test_playback_pause_api()
    test_api.teardown_method()
    print("✓ Playback pause API")
    
    test_api.setup_method()
    test_api.test_playback_seek_api()
    test_api.teardown_method()
    print("✓ Playback seek API")
    
    test_api.setup_method()
    test_api.test_playback_seek_with_offsets_api()
    test_api.teardown_method()
    print("✓ Playback seek with offsets API")
    
    test_api.setup_method()
    test_api.test_playback_invalid_action_api()
    test_api.teardown_method()
    print("✓ Playback invalid action API")
    
    test_api.setup_method()
    test_api.test_playback_seek_without_timestamp_api()
    test_api.teardown_method()
    print("✓ Playback seek without timestamp API")
    
    test_api.setup_method()
    test_api.test_playback_session_not_found_api()
    test_api.teardown_method()
    print("✓ Playback session not found API")
    
    test_api.setup_method()
    test_api.test_playback_persists_session()
    test_api.teardown_method()
    print("✓ Playback persists session")
