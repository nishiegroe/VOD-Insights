...
app = Flask(__name__)
app.register_blueprint(multi_vod_bp)
...
def vod_media_file(filename: str) -> Any:
    ...
    if file_path is None or not file_path.exists():
        abort(404)
    # Use streaming with range support
...
def api_video_stream() -> Response:
    """
    Stream a video file with HTTP range request support for seeking.
    
    Query parameters:
        path: Full file path to the video file
    ...
    file_path = resolve_vod_path(file_path_str)
    if file_path is None or not file_path.exists():
        abort(404)
