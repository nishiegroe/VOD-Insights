...
def test_resolve_existing_allowed_file_path_rejects_directory(tmp_path):
    allowed_dir = tmp_path / "allowed"
    allowed_dir.mkdir()

    resolved = resolve_existing_allowed_file_path(str(allowed_dir), [allowed_dir])

    assert resolved is None


def test_resolve_existing_allowed_file_path_accepts_existing_file(tmp_path):
    allowed_dir = tmp_path / "allowed"
    allowed_dir.mkdir()
    file_path = allowed_dir / "present.mp4"
    file_path.write_text("ok", encoding="utf-8")

    resolved = resolve_existing_allowed_file_path(str(file_path), [allowed_dir])
